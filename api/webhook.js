...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (action === 'list') {
        ...
        return res.status(200).json((data || []).map(c => ({
          telefone: c.phone_number, nome: c.contact_name || c.phone_number, emIntervencao: c.status === 'human',
          etapa: c.status, ultimaAtividade: c.last_message_time, ultima: c.last_message || 'Sem mensagens'
        })));
      }
      ...
      if (action === 'messages') {
        ...
        return res.status(200).json({
          mensagens: (messages || []).map(m => ({
            data: m.created_at, tipo: m.direction === 'outbound' ? (m.sender_type === 'human' ? 'humano' : 'bot') : 'cliente',
            mensagem: m.content, nome: m.sender_type === 'human' ? 'Voce' : (m.sender_type === 'bot' ? 'Bot' : 'Cliente')
          })),
      ...
      return res.status(200).json({ status: 'online', greeting: training?.greeting_message, bot_active: training?.active !== false });
    }
    ...
    if (req.method === 'POST') {
      ...
      // --- POST COM ACTION ---
      if (hasAction) {
        const action = req.query.action, phone = body.phone;
        if (action === 'intervene') {
          await supabase.from('conversations').upsert({ phone_number: phone, status: 'human', updated_at: new Date().toISOString() }, { onConflict: 'phone_number' });
          await saveMessage(phone, 'Atendente humano assumiu.', 'outbound', 'system');
          return res.status(200).json({ ok: true });
        }
        if (action === 'release') {
          await supabase.from('conversations').upsert({ phone_number: phone, status: 'bot', updated_at: new Date().toISOString() }, { onConflict: 'phone_number' });
      ...
      if (isWhatsApp || hasWhatsAppStructure) {
        ...
        let convStatus = 'bot';
        try {
          const { data: conv } = await supabase.from('conversations').select('status').eq('phone_number', from).single();
...
async function saveTrainingToSupabase(data) {
  try {
    ...
    if (findError) console.log('Erro buscar:', findError.message);
...
async function getTraining() {
  try {
    ...
    if (data) {
      console.log('Training do banco:', data.greeting_message?.substring(0, 50));
      return data;
    }
...
async function saveMessage(phone, content, direction, senderType, contactName = null) {
  try {
    let convId = null;
    const { data: existing } = await supabase.from('conversations').select('id').eq('phone_number', phone).maybeSingle();

    if (existing) {
      convId = existing.id;
      await supabase.from('conversations').update({
        contact_name: contactName || undefined, last_message: content, last_message_time: new Date().toISOString(),
        unread: direction === 'inbound', updated_at: new Date().toISOString()
      }).eq('id', convId);
    } else {
    ...
    await supabase.from('messages').insert({
      conversation_id: convId, phone_number: phone, direction, content, sender_type: senderType,
      created_at: new Date().toISOString()
    });
  } catch (error) { console.error('Erro saveMessage:', error.message); }
}
